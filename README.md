...
В БД хранятся пары вида: дата, событие.
  Дата имеет формат строки вида: Год-Месяц-День, 
    где Год, Месяц и День - целые читла.
  Событие имеет формат строки из произвольных печатных символов без разделителей внутри      (пробелов, табуляций и пр.).
...
Команды. доступные для считывания и выполнения в БД:
...
*Ввод в этой задаче не всегда корректен: некоторые ошибки ввода программа должна корректно обрабатывать.*
